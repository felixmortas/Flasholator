...
final class FlashcardTableProjection {
  FlashcardTableProjection(
      {required this.version, required Iterable<FlashcardTablePair> pairs})
...
final class FlashcardStatisticsProjection {
  const FlashcardStatisticsProjection(
      {required this.version, required this.data});
...
final class FlashcardCollectionProjections {
  ...
  static FlashcardReviewProjection review(FlashcardCollectionSnapshot snapshot,
          {required DateTime now}) =>
      FlashcardReviewProjection(
        version: snapshot.version,
        cards: snapshot.cards.where((card) =>
            card.nextReviewDate == null || !now.isBefore(card.nextReviewDate!)),
      );

  static FlashcardTableProjection table(FlashcardCollectionSnapshot snapshot) {
    final pairs = _selectedPairs(snapshot)
        .map(
            (pair) => FlashcardTablePair(key: pair.key, card: pair.cards.first))
  ...
  static FlashcardStatisticsProjection statistics(
    ...
    final series = counts.entries
        .map((entry) => TimeSeriesData(entry.key, entry.value))
        .toList()
    ...
    return FlashcardStatisticsProjection(
      ...
      data: StatsData(
        ...
        totalPairs: languages.length,
        dailyAverage: averages.$1,
        weeklyAverage: averages.$2,
        monthlyAverage: averages.$3,
        yearlyAverage: averages.$4,
        dailySeries: series,
  ...
  static bool _isReciprocalPair(
      PersistedFlashcard first, PersistedFlashcard second) {
    final values = [
      first.front,
      first.back,
      first.sourceLang,
    ...
    return values.every((value) => value.trim().isNotEmpty) &&
        ...
        first.back == second.front &&
        first.sourceLang == second.targetLang &&
        first.targetLang == second.sourceLang;
  }
  ...
  static List<_SelectedPair> _selectedPairs(
      FlashcardCollectionSnapshot snapshot) {
  ...
  static List<RankingItem> _ranking(
      List<PersistedFlashcard> cards, num Function(PersistedFlashcard) value) {
    final ranked = [...cards]
      ..sort((left, right) => value(right).compareTo(value(left)));
    return ranked
        .take(5)
        .map((card) => RankingItem(
            card.front, value(card), '${card.sourceLang}-${card.targetLang}'))
        .toList(growable: false);
  }
}
