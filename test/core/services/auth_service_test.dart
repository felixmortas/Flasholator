...
void main() {
  ...
  setUp(() { auth = MockFirebaseAuth(); service = AuthService(firebaseAuth: auth); });
  ...
  test('registration, login, reset and sign out keep their Firebase contracts', () async {
    ...
    verify(() => auth.createUserWithEmailAndPassword(email: 'a@example.com', password: 'pw')).called(1);
    verify(() => auth.signInWithEmailAndPassword(email: 'a@example.com', password: 'pw')).called(1);
  ...
  test('auth-state stream is forwarded and login errors remain absorbed',
    ...
    await expectLater(service.authStateChanges(), emitsInOrder([isNull, emitsDone]));
