...
void main() {
  ...
  test('round-trip Drift preserves every flashcard field', () {
    ...
    final restored = Flashcard.fromDrift(FlashcardData(
      ...
      sourceLang: companion.sourceLang.value,
      targetLang: companion.targetLang.value,
      addedDate: companion.addedDate.value,
      quality: companion.quality.value,
      easiness: companion.easiness.value,
      interval: companion.interval.value,
      repetitions: companion.repetitions.value,
      timesReviewed: companion.timesReviewed.value,
      lastReviewDate: companion.lastReviewDate.value,
      nextReviewDate: companion.nextReviewDate.value,
    ));
  ...
  test('due includes null, past and now but excludes a future date', () {
    ...
    expect(card(due: now.subtract(const Duration(seconds: 1))).isDue(now: now),
        isTrue);
    expect(card(due: now).isDue(now: now), isTrue);
    expect(card(due: now.add(const Duration(seconds: 1))).isDue(now: now),
        isFalse);
  });
}
