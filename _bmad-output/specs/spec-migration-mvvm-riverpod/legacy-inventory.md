...
| `core/providers/user_sync_provider.dart` | `features/authentication/auth_session_repository.dart` | Aucun lors du réaudit ; `userSyncStateProvider` n'avait aucun lecteur | Aucun consommateur à migrer | `test/features/authentication/auth_session_repository_test.dart` : 13 tests verts | `rg -n "user_sync_provider\|userSyncStateProvider" lib test` : aucune référence après retrait | retrait préparé | `flutter analyze` : 112 diagnostics hors retrait ; `flutter test` : 7 échecs dans `test/firestore_users_dao_test.dart` (mocks renvoyant `null`) ; commit isolé à consigner |
