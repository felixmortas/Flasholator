...
> **Contrat canonique.** Ce SPEC et `migration-contract.md` définissent complètement ce qui doit être migré, préservé et vérifié.

# Migration MVVM par fonctionnalité avec Riverpod

## Why

Ce mandat de refactoring rend Flasholator évolutif et testable sans altérer l’expérience de traduction et de répétition espacée déjà livrée aux utilisateurs. La logique métier et les intégrations sont aujourd’hui mêlées à des services et providers historiques ; la migration progressive établit des frontières vérifiables avant chaque remplacement.
...
- **CAP-2**
  ...
  - **success:** Les vues migrées ne contiennent ni logique métier ni instanciation d’infrastructure, et leurs ViewModels exposent explicitement les états de chargement, données et erreur.
- **CAP-3**
...
- La CI doit continuer à exécuter l’analyse Flutter et les tests à chaque push et pull request.
...
- Quel inventaire précis des services/providers legacy et quels critères de couverture minimaux doivent conditionner leur suppression ?
- Les erreurs aujourd’hui absorbées ou seulement journalisées doivent-elles être conservées à l’identique, ou faire l’objet d’une évolution UX explicitement priorisée ?
